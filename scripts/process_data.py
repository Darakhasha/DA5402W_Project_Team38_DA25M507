...
def clean_and_engineer(input_path: str, output_path: str):
    ...
    datetime_col = None
    for col in ['tpep_pickup_datetime', 'pickup_datetime', 'Trip_Pickup_DateTime']:
    ...
    if 'PULocationID' in df.columns:
        ...
        df['zone_id'] = (df['pickup_latitude'].round(2).astype(str) + "_" + 
    ...
    demand_df = df.groupby(['pickup_hour', 'zone_id']).agg(**agg_dict).reset_index()
    ...
    demand_df['is_weekend'] = demand_df['day_of_week'].isin([4, 5]).astype(int)
...
if __name__ == '__main__':
    # Set up default parameters for easy command prompt or Airflow triggers
    raw = sys.argv[6] if len(sys.argv) > 1 else 'data/raw/yellow_tripdata_2024-01.parquet'
    processed = sys.argv[7] if len(sys.argv) > 2 else 'data/processed/taxi_demand_features.parquet'
    clean_and_engineer(raw, processed)